...
public class AsaMath {
	 ...
	 * 
	 * @return a <code>long</code> value representing the number of minutes or hours in <code>time</code> dependant on the units specified
	 */
	 ...
	 * 
	 * @return <code>true</code> if the specified object can be parsed as a <code>double</code>; <code>false</code> otherwise
	 */
	 ...
	 * @return <tt>Double.NaN</tt> if vals is null or all values are <tt>Double.NaN</tt>, otherwise the average of <tt>vals</tt>
	 */
	 ...
	 * @param ignoreVal
	 *            the value to ignore
	 * @return <tt>Double.NaN</tt> if vals is null or all values are <tt>ignoreVal</tt>, otherwise the average of <tt>vals</tt>
	 */
	 ...
	 * @param vals
	 *            the values to be averaged
	 * @return <tt>Float.NaN</tt> if vals is null or all values are <tt>Float.NaN</tt>, otherwise the average of <tt>vals</tt>
	 */
	 ...
	 * @param ignoreVal
	 *            the value to ignore
	 * @return <tt>Float.NaN</tt> if vals is null or all values are <tt>ignoreVal</tt>, otherwise the average of <tt>vals</tt>
	 */
	...
	/**
	 * Returns the statistical mode of the given dataset denoted by <code>numbers</code> while ignoring entries whose values match
	 ...
	 * 
	 * @return an array of values which share the same frequency as the highest occuring value from the array <code>numbers</code>
	 * 
	 */
	...
	/**
	 * Rounds <code>value</code> to the closest multiple of {@code nearest}. The {@code roundDirection} parameter indicates which direction
	 * to force rounding. A '0' rounds naturally (up for values > 0.5, down for values < 0.5). A number > 0 forces a round up, while < 0
	 * forces a round down.
	 * <p>
	 * Examples:
	 * </p>
	 ...
	 *            the direction to force rounding. <i>0</i> uses natural rounding, <i><0</i> always rounds up, <i>>0</i> always rounds down.
	 * 
