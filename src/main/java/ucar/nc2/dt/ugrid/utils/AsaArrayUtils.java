...
public class AsaArrayUtils {
	...
	/**
	 * Recreates the array specified by <code>original</code> as a two dimensional set with dimension sizes defined by {@code dim1} and
	 * {@code dim2}. The values of this array are then swapped along the newly defined grid and returned as a single dimension array.
	 * 
	 * <p>
	 * <b>For example:</b> <br />
	 ...
	 * <code>tolerance</code>. Because this returns the first value within range, when closestPrevious() is invoked for the array defined by
	 * {5l, 6l, 7l, 8l, 9l} where <code>checkVal = 8</code> and <code>tolerance = 2</code>, the index 1 (value 6) will be returned.
	 * 
	 * @param myArray
	 ...
	 *         <code>checkVal - tolerance</code>. If no value is within this tolerance range and less then <code>checkVal</code> or all
	...
	/**
	 * Retrieves a subset of the given array ignoring values with an index lesser than <code>start</code> and pulling values at an interval
	 ...
	 * 
	 * @return an array subset from indices <code>start</code> to <code>in.length</code> at an interval denoted by <code>skip</code>
	 */
