 ...
 * <ul>
 * <li>Fast intersection query performance. To achieve this, the RTree uses only main memory to store entries. Obviously this will only
 ...
 *
 * <p>
 * The main reason for the high speed of this RTree implementation is the avoidance of the creation of unnecessary objects, mainly achieved
 * by using primitive collections from the COLT library.
 * </p>
 *
 * NOTE: Node.findEntry() is not implemented and recalculateMBR() is not implemented. These are used in node deletion, therefore nodes
...
public class RTree implements SpatialIndex, Serializable {
	...
	/**
	 * Inner class used as a bridge (adapter) between the COLT TIntProcedure and the SpatialIndex IntProcedure. This is used because the
	 ...
	 * <ul>
	 * <li>MaxNodeEntries</li> This specifies the maximum number of entries in a node. The default value is 10, which is used if the
	 * property is not specified, or is less than 2.
	 * <li>MinNodeEntries</li> This specifies the minimum number of entries in a node. The default value is half of the MaxNodeEntries value
	 ...
	 * @return the index of the nearest neighbor to the input point. returns -1 if no points are found within the searchRadius.
	 */
	...
	/**
	 * Convenience method for nearestNeighbor search where the IntProcedure is handled internally. Uses the default search radius of 0.5
	 * units.
	 *
	 * @param p
	 *            the query point.
	 * @return the index of the nearest neighbor to the input point. returns -1 if no points are found within the search radius.
	 */
	...
	/**
	 * Finds the entry (polygon) which is closest to the given point, and invokes apply() on the given <code>IntProcedure</code> for that
	 * entry. Any entries found with a distance greater than 0.5 will not be returned. Also, if two or more entries are equidistant from the
	 ...
	 * @param searchRadius
	 *            The furthest distance away from a polygon to search. Polygons further than this will not be found. This should be as small
	 *            as possible to minimize the search time. Use Double.POSITIVE_INFINITY to guarantee that the nearest polygon is found, no
	...
	public void nearest(final LatLonPoint2D p, IntProcedure v, double searchRadius) {
		...
		if (nearestEntriesMap.size() > 1) {
			...
			class FindClosestEntryProc implements cern.colt.function.IntProcedure, Serializable {
				...
				/**
				 * Sets <code>closestIndex</code> to the specified key if the centroid of that keys polygon is closer than the last one
				 * checked.
				 */
	...
	/**
	 * Convenience method for intersects search where the IntProcedure is handled internally. Returns only those cells that are completely
	...
	/**
	 * Convenience method for contains search where the IntProcedure is handled internally. Returns only those cells that are completely
	 * contained within the <code>r</code> polygon.
	 *
	 * @param r
	 *            the bounding polygon to search within
	...
	private Node splitNode(Node n, LatLonPolygon2D newRect, int newId) {
		 ...
		 * QS2 [Check if done]
		 *
		 * If all entries have been assigned, stop. If one group has so few entries that all the rest must be assigned to it in order for it
		...
		while (n.entryCount + newNode.entryCount < maxNodeEntries + 1) {
			 ...
			 *
			 * Invoke algorithm pickNext() to choose the next entry to assign. Add it to the group whose covering rectangle will have to be
			 * enlarged least to accommodate it. Resolve ties [equal area differences] by adding the entry to the group with smaller area,
	...
	/**
	 * Pick the seeds used to split a node. Select two entries to be the first elements of the groups. These two entries would be the ones
	 * with the largest wasted space (greatest bounding rectangle) if they were in the same group. By using these two entries as seeds, they
	...
	private void pickSeeds(Node n, LatLonPolygon2D newRect, int newId, Node newNode) {
		 ...
		 *
		 * Along each dimension, find the entry whose rectangle has the highest low side, and the one with the lowest high side. Record the
		 * separation.
		 *
		 * [TPL] This has been modified from the Gutman RTree abstract such that separations along various dimensions are not normalized
		 * until the maximal separations are determined. Then from those separations the indices of the most extreme pairs are determined as
		 * if they were normalized by comparing the values unto which they would be normalized (the mbr's widths in each dimension).
		 *
		 * Normalizing in this fashion removes some of the mathematics from being performed every iteration, thereby optimizing rtee
		 * insertion speed.
		 */
	...
	// *
	// * [Determine cost of putting each entry in each group] For each entry not yet in a group, calculate the area increase required in the
	...
	/**
	 * Recursively searches the tree for the nearest entry. Other queries call execute() on an IntProcedure when a matching entry is found;
	 * however nearest() must store the entry Ids as it searches the tree, in case a nearer entry is found. Uses the member variable
	 * nearestIds to store the nearest entry IDs. Since some nearest entries may be equiquadistant to the given point, the entries polygon
	 * is cached in the variable nearestEntriesMap so that the calling method may (if needed) sort the entries further by centroid or other
		...
		while (n.level != level) {
			...
			for (int i = 1; i < n.entryCount; i++) {
				...
				if ((tempEnlargement < leastEnlargement)
					|| ((tempEnlargement == leastEnlargement) && (tempPoly.getBouningLatLonRectangle2D().getArea() < n.getEntry(index)
						.getBouningLatLonRectangle2D().getArea()))) {
					index = i;
	...
	private void checkConsistency(int nodeId, int expectedLevel, LatLonPolygon2D expectedMBR) {
		...
		if (n.level != expectedLevel) {
			logger.error("Error: Node " + nodeId + " of " + getNodeCount() + ", expected level " + expectedLevel + ", actual level " + n.level);
		}

		LatLonRectangle2D calculatedMBR = calculateMBR(n);

		if (!n.mbr.equals(calculatedMBR)) {
			logger.error("Error: Node: " + nodeId + " of " + getNodeCount() + " at level: " + expectedLevel + ", calculated MBR does not equal stored MBR");
		}

		if (expectedMBR != null && !n.mbr.equals(expectedMBR.getBouningLatLonRectangle2D())) {
			logger.error("Error: Node " + nodeId + " of " + getNodeCount() + " at level: " + expectedLevel + ", expected MBR (from parent) does not equal stored MBR");
		}
		...
		if ((expectedMBR != null) && ((Object) n.mbr == (Object) expectedMBR)) {
			logger.error("Error: Node " + nodeId + " of " + getNodeCount() + " at level: " + expectedLevel + " MBR using same rectangle object as parent's entry");
		}

		/* Recursively check all children of this node */
		for (int i = 0; i < n.entryCount; i++) {
			if (n.entries[i] == null) {
				logger.error("Error: Node " + nodeId + " of " + getNodeCount() + " at level: " + expectedLevel + ", Entry " + i + " is null");
			}
	...
	/**
	 * Builds a 2 length double array containing the minimum x and y values from the given rectangle. The 0 index of this array contains the
	...
	/**
	 * Builds a 2 length double array containing the maximum x and y values from the given rectangle. The 0 index of this array contains the
	...
	/**
	 * Produces a union of the two rectangles, and determines how much <code>src</code> would have to be enlarged to encompass this union.
	 * The original source rectangles will remain unchanged.<br />
	 * <br />
	 * If <code>src</code> fully contains <code>addition</code> this will always return 0, since <code>src</code> would not have to be
	 * enlarged.
	 *
	 * @param src
	 *            a source rectangle
	 ...
	 *
	 * @return the area which <code>src1</code> would have to be enlarged to, in order to encompass the union of itself and another
...
//		return getEnlargementArea(srcVals, addVals);
	...
	public static double getEnlargementArea(final double[] src, final double[] addition) {
		...
		return getEnlargementArea(new LatLonRectangle2D(src[0],src[1],src[2],src[3]), new LatLonRectangle2D(addition[0],addition[1],addition[2],addition[3]));
...
//		return newArea - oldArea;
	...
	/**
	 * Constructs a string representation of this <code>RTree</code> as a tab-indented node list. If this <code>RTree</code> contains many
	...
	public String toString() {
		return this.getClass().getSimpleName() + " {entries:" + this.size + ", nodes:" + this.nodeMap.size() + ", tree-height:"
			+ this.treeHeight + ", max-node-entries:" + this.maxNodeEntries + ", min-node-entries:" + this.minNodeEntries + "}";
	}



	/**
	 * Recursively builds a string representation of the given rtree starting at nodeId. Starting at node id rtree.getRootNodeId() is
	 * recommended.
	 *
	 * @param rtree
