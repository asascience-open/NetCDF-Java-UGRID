...
public class LatLonRectangle2D implements Serializable {
	...
	public LatLonRectangle2D(LatLonPoint2D leftPoint, double dLat, double dLon) {
		this(leftPoint.getLatitude(), leftPoint.getLongitude(), dLat + leftPoint.getLatitude(), LatLonPoint2D.normLon(dLon
	...
	/**
	 * Returns the coordinates of this <code>LatLonRectangle2D</code> as a <code>double</code> array with a length of 4. The coordinates are
	 ...
	 * 
	 * @return a <code>double</code> array with a length of 4 containing the lat/lon points of this rectangle. If this object contains no
	...
	public boolean contains(LatLonRectangle2D rect) {
		return (width >= rect.getWidth()) && this.contains(rect.getLowerLeftPoint()) && this.contains(rect.getUpperRightPoint());
	}
	...
	/**
	 * Extends the bounds of this <code>LatLonRectangle2D</code> by the coordinates in the bounding rectangle given by <code>bounds</code>.
	 * It is assumed that bounds is a non-null double array with a length of 4 which contains lat/lon coords in the arrangement of LLy, LLx,
	 * URy, URx.
	 * 
	 * @param rect
	 * @param bounds
	...
	public void extend(double[] bounds) {
		...
		lonMin = Math.min(lonMin, nLonMin); /* Dont normalize lons here, they are auto normalized on LatLonPoint2D.Double.setLongitude() */
		lonMax = Math.max(lonMax, nLonMax); /* Dont normalize lons here, they are auto normalized on LatLonPoint2D.Double.setLongitude() */
...
//		}
	...
	public boolean shouldCrossDateline(LatLonPolygon2D poly) {
		...
		// if ((polyC.getLongitude() > 0 & rectC.getLongitude() > 0) || (polyC.getLongitude() < 0 & rectC.getLongitude() < 0)) {
		// return false;
		// }
		//		
	...
	public LatLonRectangle2D translate(double deltaLat, double deltaLon) {
		init(this.getLowerLeftPoint().getLatitude() + deltaLat, this.getLowerLeftPoint().getLongitude() + deltaLon, this
			.getUpperRightPoint().getLatitude()
			+ deltaLat, this.getUpperRightPoint().getLongitude() + deltaLon);
		return this;
	}

	/**
	 * Uses {@link #translate(double, double)} to shift this <tt>LatLonRectangle2D</tt> so that the centroid matches the centroid of the
	...
	/**
	 * Uses {@link #translate(double, double)} to shift this <tt>LatLonRectangle2D</tt> so that the centroid matches <b>newCenterPoint</b>.
	...
	/**
	 * Uses {@link #translate(double, double)} to shift this <tt>LatLonRectangle2D</tt> so that the centroid matches the location specified
  ...
  public LatLonRect toLatLonRect() {
    return new LatLonRect(new LatLonPointImpl(this.getUpperLeftPoint().getLatitude(), this.getUpperLeftPoint().getLongitude()), new LatLonPointImpl(this.getCenterLatitude(), this.getCenterLongitude()));
  }
