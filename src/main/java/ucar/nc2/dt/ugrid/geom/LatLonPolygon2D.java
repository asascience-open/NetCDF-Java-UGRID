 ...
 */
  ...
  public abstract class LatLonPolygon2D implements Serializable {
	...
	public boolean contains(LatLonRectangle2D rect) {
		return contains(rect.getUpperLeftPoint()) && contains(rect.getUpperRightPoint()) && contains(rect.getLowerRightPoint())
			&& rect.contains(rect.getLowerLeftPoint());
	}
	...
	/**
	 * Calculates the bounding box of this <code>LatLonPolygon2D</code> and returns the coordinates as a <code>double</code> array with a
	 * length of 4. The coordinates are arranged in the order of western lon, southern lat, eastern lon, and northern lat.<br />
	 * <br />
...
//			vertices.add(new LatLonPoint2D.Double(lat, lon));
		...
		/**
		 * Create a new polygon from the provided arrays. If the arrays are not the same length, the resulting polygon is empty. The
		...
		/**
		 * Calculates the bounding box of this <code>LatLonPolygon2D</code> and returns the coordinates as a <code>double</code> array with
		 * a length of 4. The coordinates are arranged in the order of southern lat, western lon, northern lat, and eastern lon<br />
		 * <br />
		 * In terms of points where LL = lower left and UR = upper right, this is equivalent to the arrangement of
		 * <code>double[] {lly, llx, ury, urx}</code>.
		 *
		...
		public boolean intersects(LatLonPoint2D point, boolean crossesDateline) {
			...
			for (int i = 1; i < count; i++) {
				vlon0 =
					(crossesDateline) ? LatLonPoint2D.normLon360(vertices.get(i - 1).getLongitude()) : vertices.get(i - 1).getLongitude();
				vlon1 = (crossesDateline) ? LatLonPoint2D.normLon360(vertices.get(i).getLongitude()) : vertices.get(i).getLongitude();
				if (lineIntersectsPoint(vlon0, vertices.get(i - 1).getLatitude(), vlon1, vertices.get(i).getLatitude(), px, py)) {
					return true;
				}
			}
			/*
			...
			if (count > 2) {
				vlon0 =
					(crossesDateline) ? LatLonPoint2D.normLon360(vertices.get(count - 1).getLongitude()) : vertices.get(count - 1)
						.getLongitude();
				vlon1 = (crossesDateline) ? LatLonPoint2D.normLon360(vertices.get(0).getLongitude()) : vertices.get(0).getLongitude();
				if (lineIntersectsPoint(vlon0, vertices.get(count - 1).getLatitude(), vlon1, vertices.get(0).getLatitude(), px, py)) {
					return true;
				}
			}
		...
		public boolean intersects(LatLonPoint2D e0, LatLonPoint2D e1, boolean crossesDateline) {
			...
			if (count == 1) {
				...
				if (crossesDateline) {
					return lineIntersectsPoint(LatLonPoint2D.normLon360(e0.getX()), e0.getY(), LatLonPoint2D.normLon360(e1.getX()), e1
						.getY(), LatLonPoint2D.normLon360(vertices.get(0).getLongitude()), vertices.get(0).getLatitude());
				}
				return lineIntersectsPoint(e0.getX(), e0.getY(), e1.getX(), e1.getY(), vertices.get(0).getLongitude(), vertices.get(0)
					.getLatitude());
			}
			...
			for (int i = 1; i < count; i++) {
				p2x = (crossesDateline) ? LatLonPoint2D.normLon360(vertices.get(i - 1).getLongitude()) : vertices.get(i - 1).getLongitude();
				p2y = vertices.get(i - 1).getLatitude();

				p3x = (crossesDateline) ? LatLonPoint2D.normLon360(vertices.get(i).getLongitude()) : vertices.get(i).getLongitude();
			 ...
			 */
			if (count > 2) {
				p2x = (crossesDateline) ? LatLonPoint2D.normLon360(vertices.get(0).getLongitude()) : vertices.get(0).getLongitude();
		...
		public double distanceSq(LatLonPoint2D p) {
			...
			if (getVertexCount() <= 0) {
				...
				for (int i = 0; i < count - 1; i++) {
					v0x = (crossesDateline) ? LatLonPoint2D.normLon360(vertices.get(i).getLongitude()) : vertices.get(i).getLongitude();
					v0y = vertices.get(i).getLatitude();
					v1x =
						(crossesDateline) ? LatLonPoint2D.normLon360(vertices.get(i + 1).getLongitude()) : vertices.get(i + 1)
							.getLongitude();
				...
				v0x = (crossesDateline) ? LatLonPoint2D.normLon360(vertices.get(0).getLongitude()) : vertices.get(0).getLongitude();
