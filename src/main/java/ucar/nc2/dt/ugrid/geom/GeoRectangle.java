...
/**
 * The <code>GeoRectangle</code> class provides a mechanism for manipulating rectangular regions on a geographically referenced plane.
 * 
...
public abstract class GeoRectangle implements Shape, Serializable {
	...
	/**
	 * Adds a <code>GeoRectangle</code> object to this <code>GeoRectangle</code>. The resulting <code>GeoRectangle</code> is the union of
	...
	public boolean contains(GeoRectangle r) {
		...
		return (r.getMinX() >= getMinX() && r.getMinY() >= getMinY() && r.getMaxX() <= getMaxX() && r.getMaxY() <= getMaxY());
	}
	...
	/**
	 * Retrieves a copy of this <code>GeoRectangle</code> object with the exact same bounding coordinates. The backing polygon associated
	...
	/**
	 * Set the coordinates of this rectangle to the coordinates of the two points. These points should be "opposite" corners of the
	 * rectangle.
	 * 
	 * @param corner1
	 ...
	 * <code>Rectangle2D</code> object. One of the source rectangles can also be the destination to avoid creating a third Rectangle2D
	...
	public static class Double extends GeoRectangle {
		...
		/**
		 * Constructor accepting two double arrays, one for x values and one for y values. Only the FIRST TWO members of each array are
		 * used.
		 * 
		 * @param xVals
		...
		/**
		 * Adds a <code>Rectangle2D</code> object to this <code>Rectangle2D</code>. The resulting <code>Rectangle2D</code> is the union of
		 * the two <code>GeoRectangle</code> objects.
		 * 
		 * @param r
		 *            the <code>Rectangle2D</code> to add to this <code>Rectangle2D</code>.
		...
		/**
		 * Determines if the given object possesses the same coordinate bounds as this <code>GeoRectangle</code>. The two objects do not
		 * necessarily have to be equivalent as given by <code>this == obj</code>, however, if they are this will return true
		 * 
		 * @param obj
		 *            another <code>Object</code>
		 * 
		 * @return <code>true</code> if the given object is an instance of <code>GeoRectangle</code> and has the same rectangular bounds as
