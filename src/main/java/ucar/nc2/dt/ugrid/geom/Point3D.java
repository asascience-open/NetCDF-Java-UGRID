 ...
 * <p>
 * This class is only the abstract superclass for all objects that store a 3D coordinate. The actual storage representation of the
...
public abstract class Point3D implements Cloneable {
	...
	public static class Float extends Point3D implements Serializable {
		 ...
		 * <p>
		 * NOTE: Units are NOT considered. The units of the {@code origin} and {@code (u, v, w)} components are assumed to be the same.<br>
	 ...
	 * instantiation and provide a number of formats for storing the information necessary to satisfy the various accessor methods below.
	 * 
	...
	/**
	 * Determines whether or not two points are equal. Two instances of <code>Point3D</code> are equal if the values of their <code>x</code>
	 * , <code>y</code> and <code>z</code> member fields, representing their position in the coordinate space, are the same.
	 * 
	 * @param obj
	 *            an object to be compared with this <code>Point3D</code>
	 * @return <code>true</code> if the object to be compared is an instance of <code>Point3D</code> and has the same values;
