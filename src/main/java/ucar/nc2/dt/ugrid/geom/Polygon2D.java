...
/**
 * NOTE: This class should NOT be used in situations involving geographic (lat, lon) data. In such cases, use LatLonPolygon2D instead.
 * 
...
/**
	...
	public static boolean linesIntersect(double p0x, double p0y, double p1x, double p1y, double p2x, double p2y, double p3x, double p3y) {
		 ...
		 */
		double a1 =
			((p0x * p2y) - (p0x * p3y) - (p2x * p0y) + (p2x * p3y) + (p3x * p0y) - (p3x * p2y))
				/ ((p1x * p2y) - (p1x * p3y) - (p2x * p1y) + (p2x * p3y) + (p3x * p1y) - (p3x * p2y));
		double a2 =
			((p1x * p0y) - (p1x * p3y) - (p0x * p1y) + (p0x * p3y) + (p3x * p1y) - (p3x * p0y))
				/ ((p1x * p2y) - (p1x * p3y) - (p2x * p1y) + (p2x * p3y) + (p3x * p1y) - (p3x * p2y));
		...
		// }
		/**
		 * According to the 2D Segments Intersection test given by figure 5 page 4 at http://wscg.zcu.cz/WSCG2004/Papers_2004_Full/B83.pdf,
		 * if the sign of the barycentric coordinates of p1, p2, and p3 with respect to p4 are a1<=0, a2>=0, and a3>=0 then the line
		 * segments given by (p1, p4) and (p2, p3) intersect.
		 * 
		 * NOTE: To save processing time, a3 is not calculated. Since a1 + a2 + a3 = 1 it can be said that a3 = 1 - a1 - a2. Additionally,
		 * for this check to be true a3 must be greater than or equal to 1. Instead of writing a3 >= 1 we can check that a1 + a2 <= 1
		 */
		// if (a1 <= 0 && a2 >= 0 && a3 >= 0) {
	...
	public static boolean lineIntersectsPoint(double e0x, double e0y, double e1x, double e1y, double p1x, double p1y) {
		/**
		 * The following calculation normalizes all points (p1, e0, e1) over e0 and checks if vector e1 is divisible by p1 via:
		 * 
		 * p1x / p1y == e1x / e1y
		 * 
		 ...
		 * e1x / p1x * p1y == e1y
		 * 
		 * At this point, it is understood that p1 is intersected by the infinitely extending line given by e0, e1. Simply checking if p1 is
	...
	/**
	 * Close the polygon. No further segments can be added. If this method not called, then the path iterators will treat the polygon as
	 * thought it were closed, and implicitly join the most recently added vertex to the first one. However, this method should generally be
	...
	/**
	 * Return true if the given point is inside the polygon. This method uses a straight-forward algorithm, where a point is taken to be
	...
	/**
	 * Return true if the given rectangle is entirely inside the polygon. (Currently, this algorithm can be fooled by supplying a rectangle
	 ...
	 * In this instance, although the polygons intersect the given polygon is not "wholely" contained within this polygon. This method would
	 ...
	 *            a <code>Polygon2D</code> object
	 * 
	 * @return <code>true</code> if all the points of the given polygon are contained within this Polygon2D, otherwise <code>false</code>
	 */
	...
	/**
	 * Calculates the square of the distance from the point <code>p</code> to the nearest line segment specified by connecting the vertices
	...
	public double distanceSq(Point2D p) {
		...
		if (getVertexCount() <= 0) {
			...
			tempDistSq = Line2D.ptSegDistSq(getX(0), getY(0), getX(getVertexCount() - 1), getY(getVertexCount() - 1), p.getX(), p.getY());
			if (tempDistSq < resultSq) {
				resultSq = tempDistSq;
			}
	...
	/**
	 * Checks if the line segment given by the extremes <code>e1</code> and <code>e2</code> intersect this polygon, by checking segment to
	...
	/**
	 * Checks if this polygon intersects the given polygon. This algorithm uses the 2D Polygon-Polygon Intersection test as proposed in the
	 * webaddress below
	 * 
	 * @return <code>true</code> if there is an intersection, otherwise <code>false</code>
	 * 
	 * @see <a href="http://wscg.zcu.cz/WSCG2004/Papers_2004_Full/B83.pdf"> Efficient Collision Detection between 2D Polygons</a>
	 */
	...
	/**
	 * Test if the polygon is intersected by the given rectangle. (Currently, this algorithm can be fooled by supplying a rectangle that has
	...
	/**
	 * Test if the polygon is intersected by the given GeoRectangle. (Currently, this algorithm can be fooled by supplying a rectangle that
	 * has no corners inside the polygon, and does not contain any vertex of the polygon, but which intersects some edges.)
	 */
	 ...
	 *            a <code>Point2D</code>
	 * 
	 * @return <code>true</code> if the given point is intersected by this <code>Polygon2D</code>, otherwise <code>false</code>
	 */
	...
	public static class Float extends Polygon2D {
		...
		/**
		 * Create a new polygon with the provided x & y locations. If the two arrays are of unequal length, an empty polygon is created. NaN
		 * values are ignored
		 * 
		 * @param xlocs
		...
		public Polygon2D.Float copy() {
			...
			// result.closePath();
			// return result;
			return new Polygon2D.Float(AsaArrayUtils.doubleArrayToFloatArray(this.getXCoords()), AsaArrayUtils.doubleArrayToFloatArray(this
				.getYCoords()));
		}
